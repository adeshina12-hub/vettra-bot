...
export async function sendOpportunityAlert(opp) {
    ...
    const message = `${emoji} *${escapeMd(opp.title)}*\n` +
...
export function startUnifiedBot() {
    if (!bot) {
    ...
    bot.hears("Research a project", async (ctx) => {
        ...
        await ctx.reply("🔎 *Research mode*\n\nSend me a project name or ticker, for example:\n`Ethereum`\n`AAVE`\n`Uniswap`\n\nI will gather the available signals and return a scored verdict.", { parse_mode: "Markdown", ...mainMenu(true) });
    });
...
async function runResearchCommand(ctx, query) {
    if (!query) {
...
function formatEarlyResults(scan) {
    if (!scan.projects.length)
        return "No projects found in the current feed\\.";
