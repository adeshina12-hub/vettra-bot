...
function normalizeFinding(finding: Partial<SecurityAuditFinding>): SecurityAuditFinding {
  const severities = new Set(["critical", "high", "medium", "low", "info"]);
  const confidence = new Set(["high", "medium", "low"]);
  return {
    severity: severities.has(finding.severity ?? "") ? finding.severity as SecurityAuditFinding["severity"] : "info",
    ...
    confidence: confidence.has(finding.confidence ?? "") ? finding.confidence as SecurityAuditFinding["confidence"] : "low",
  };
}

export async function runAudit(source: string): Promise<SecurityAuditReport> {
  const normalizedSource = source.trim();
  if (!normalizedSource) throw new Error("Contract source is required");
  if (normalizedSource.length > MAX_SOURCE_LENGTH) throw new Error(`Contract source is too large (maximum ${MAX_SOURCE_LENGTH} characters)`);
  ...
  return {
    ...
    riskRating: riskRatings.has(result.primary.riskRating ?? "") ? result.primary.riskRating as SecurityAuditReport["riskRating"] : "info",
